...
public final class FabricNetwork {
    ...
    public record GlitchPayload(String victim) implements CustomPayload {
        ...
        public static final net.minecraft.network.codec.StreamCodec<net.minecraft.network.RegistryByteBuf, GlitchPayload> STREAM_CODEC =
            net.minecraft.network.codec.StreamCodec.composite(PacketCodecs.STRING, GlitchPayload::victim, GlitchPayload::new);
    ...
    public static void sendGlitch(net.minecraft.server.world.ServerWorld world, ServerPlayerEntity victim) {
        for (ServerPlayerEntity p : world.getServer().getPlayerManager().getPlayerList()) {
            if (ServerPlayNetworking.canSend(p, GLITCH)) ServerPlayNetworking.send(p, new GlitchPayload(victim.getName().getString()));
        }
    }
}
